...
/**
 * Panel para que el usuario ingrese la Gramática Libre de Contexto (GLC).
 */
public class GLCPanel extends JPanel {

    public static final String CMD_VERIFICAR_GLC = "VERIFICAR_GLC";
    ...
    public GLCPanel(ActionListener listener) {
        ...
        JLabel title = new JLabel("Definición y Verificación de Gramática Libre de Contexto (GLC)");
    ...
    private JPanel createDefinicionPanel(ActionListener listener) {
        ...
        // --- Producciones (P) ---
        JLabel lblProducciones = new JLabel("Producciones (P): Formato: A -> aSa | b (una por línea, use '|' para alternativas, 'ε' para epsilon)");
        ...
        btnVerificar.setActionCommand(CMD_VERIFICAR_GLC);
        btnVerificar.addActionListener(listener);
        btnVerificar.setBackground(new Color(230, 126, 34)); // Naranja
    ...
    private void cargarEjemploDefecto() {
        txtVariables.setText("S");
        txtTerminales.setText("a, b");
        txtSimboloInicial.setText("S");
        txtProducciones.setText(
                "S -> aSa | b"
        );
        txtCadenaPrueba.setText("aabaa");
        mostrarResultado("Ejemplo de Gramática Libre de Contexto (GLC) cargado.\nGLC: L = { a^n b a^n | n >= 0 }.\nPresione VERIFICAR CADENA para iniciar.");
    }
